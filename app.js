...
function productImage(p){
  return p.image
    ? `<img src="${p.image}" alt="${p.name} ${p.brand}" class="product-photo" loading="lazy">`
    : p.emoji;
}
  ...
  productsEl.innerHTML = list.map(p => `
    <article class="product-card" onclick="openProduct(${p.id})">
      ...
      <div class="product-body">
        ...
        <button class="add-btn" onclick="event.stopPropagation(); openProduct(${p.id})">Ver producto</button>
      </div>
    </article>
  `).join("");
}
...
function openProduct(id){
  ...
  modal.style.cssText = `position:fixed;inset:0;background:rgba(0,0,0,.65);display:flex;align-items:center;justify-content:center;z-index:9999;padding:20px;`;
  const visual = p.image
    ? `<img src="${p.image}" alt="${p.name}" style="width:100%;height:260px;object-fit:contain;border-radius:18px;background:#fff4ee;">`
  ...
  modal.innerHTML = `
    <div style="background:white;width:100%;max-width:420px;border-radius:24px;padding:20px;text-align:center;max-height:90vh;overflow:auto;">
      ...
      <a href="https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}" target="_blank" style="display:block;background:#25D366;color:white;text-decoration:none;padding:15px;border-radius:14px;font-weight:bold;">📲 Consultar precio por WhatsApp</a>
      <button onclick="document.getElementById('productModal').remove()" style="margin-top:12px;border:0;background:none;padding:10px;">← Volver al catálogo</button>
    </div>`;
  document.body.appendChild(modal);
}

function save(){ localStorage.setItem("jys-cart", JSON.stringify(cart)); }
function addToCart(id){ cart[id]=(cart[id]||0)+1; save(); renderCart(); cartEl.classList.add("open"); }
function changeQty(id,delta){ cart[id]=(cart[id]||0)+delta; if(cart[id]<=0) delete cart[id]; save(); renderCart(); }
function renderCart(){
  const entries=Object.entries(cart);
  cartItemsEl.innerHTML=!entries.length ? `<p class="empty">Todavía no agregaste productos.</p>` :
    entries.map(([id,qty])=>{const p=PRODUCTS.find(x=>x.id===Number(id));return `<div class="cart-row"><div><strong>${p.name}</strong><br><small>${p.brand} · ${money(p.price)}</small></div><div class="qty"><button onclick="changeQty(${p.id},-1)">−</button><span>${qty}</span><button onclick="changeQty(${p.id},1)">+</button></div></div>`}).join("");
  const count=entries.reduce((s,[,q])=>s+q,0);
  const total=entries.reduce((s,[id,q])=>{const p=PRODUCTS.find(x=>x.id===Number(id));return s+(p.price||0)*q;},0);
  cartCountEl.textContent=count; cartTotalEl.textContent=money(total);
}
function sendWhatsApp(){
  const entries=Object.entries(cart);
  if(!entries.length){alert("Agregá al menos un producto al pedido.");return;}
  const lines=entries.map(([id,qty])=>{const p=PRODUCTS.find(x=>x.id===Number(id));return `• ${qty} x ${p.name} (${p.brand})`;});
  const total=entries.reduce((s,[id,q])=>{const p=PRODUCTS.find(x=>x.id===Number(id));return s+(p.price||0)*q;},0);
